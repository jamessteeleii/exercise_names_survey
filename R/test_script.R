...
#   mutate(total = sum(n))
#
# tf_recognise_plot <- ggplot(recognise_words, aes(n/total, fill = recognise)) +
#   geom_histogram(show.legend = FALSE) +
...
#   ungroup()
#
# lm <- lm(log10_tf ~ log10_rank * book_exercise_name * recognise, data = freq_by_rank)
#
# zipf_preds <- predictions(lm,
#                           variables = list(book_exercise_name = unique(freq_by_rank$book_exercise_name),
#                                            recognise = c("NO","YES")))
...
#             linetype = "dashed") +
#   labs(y = "Term Frequency (proportion on log10 scale)",
...
#     y = NULL,
#     title = "Term Frequency - Inverse Exercise Frequency",
...
#
  bigram_counts <- data_bigrams |>
    filter(recognise == "YES") |>
    filter(book_exercise_name == "flat barbell bench press") |>
...
#                   show.legend = FALSE,
#                   strength = 0.5,
#                   arrow = arrow(angle = 20, type = "closed", length = unit(4, 'mm')))  +
#     geom_node_point(color = NA, size = 10) +
#     # geom_node_label(aes(label = name), size = 5, label.padding = unit(0.5, "lines")) +
#     geom_node_text(aes(label = name), size = 5) +
#     # labs(title = paste(i)) +
#     annotate(
#       geom = "label", x = 1, y = 1.25, label = "Top", hjust = 0, vjust = 1,
#       size = 5
#     ) +
...
# ) +
#   plot_annotation(title = "Top five bigrams by frequency",
#                   caption = "Note, bigrams with higher frequencies indicated by darker edges between nodes")
#
# ggsave("bigram_plots.png", width = 15, height = 12.5, dpi = 300)
#
